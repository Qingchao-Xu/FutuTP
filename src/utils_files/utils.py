...
def visualize_goals_2D(mapping, future_frame_num, loss=None, labels: np.ndarray = None,
    ...
    fig_scale = 1.0
    marker_size_scale = 2
    target_agent_color, target_agent_edge_color = '#eca154', '#d33e4c'
            ...
            if i == 0:
                ...
                if plot_last_obs:
                    plt.plot(traj[-2], traj[-1], markersize=10 * marker_size_scale, color=target_agent_color, marker='o',
                            markeredgecolor=target_agent_color)

        for i in range(len(mapping['dense_agent_labels'])):
            if i == 0:
                continue
            plt.plot(mapping['dense_agent_trajs'][i, :, 0], mapping['dense_agent_trajs'][i, :, 1],
                     linestyle="-", color="MediumSlateBlue", marker=None, alpha=1,
                     linewidth=linewidth, zorder=0)
            temp = np.array([[mapping['dense_agent_trajs'][i, -1, 0], mapping['dense_agent_trajs'][i, -1, 1]],
                            [mapping['dense_agent_labels'][i, 0, 0], mapping['dense_agent_labels'][i, 0, 1]]])
            plt.plot(temp[:, 0], temp[:, 1],
                     linestyle="-", color="MediumSlateBlue", marker=None, alpha=1,
                     linewidth=linewidth, zorder=0)
            plt.plot(mapping['dense_agent_labels'][i, :, 0], mapping['dense_agent_labels'][i, :, 1],
                     linestyle="-", color="MediumSlateBlue", marker=None, alpha=1,
                     linewidth=linewidth, zorder=0)
            plt.plot(mapping['dense_agent_labels'][i, -1, 0], mapping['dense_agent_labels'][i, -1, 1],
                     markersize=10 * marker_size_scale, color="MediumSlateBlue", marker='o',
    ...
    if True:
        for each in predict:
            ...
            function2 = plt.plot(new_each[:, 0], new_each[:, 1], linestyle="-", color="#007672", marker=None,
                                 linewidth=linewidth)
            if add_end:
                plt.plot(each[-1, 0], each[-1, 1], markersize=10 * marker_size_scale, color="#007672", marker="o",
                         markeredgecolor="#007672")

        if add_end and "visualize_test" not in args.other_params:
            plt.plot(labels[-2], labels[-1], markersize=10 * marker_size_scale, color=target_agent_edge_color, marker="o",
                     markeredgecolor=target_agent_edge_color)
        if "visualize_test" not in args.other_params:
            function1 = plt.plot(labels[0::2], labels[1::2], linestyle="-", color=target_agent_edge_color, linewidth=linewidth)
...
def visualize_gifs(index, mapping, future_frame_num, labels: np.ndarray = None,
                        predict: np.ndarray = None, mode_num = 6):
    print("mapping['file_name']",mapping['file_name'])
    plot_last_obs = True
    assert predict is not None
    ...
    for frame in range(1, 50):
        ...
        def draw_his_trajs():
            ...
            for i, traj in enumerate(trajs):
                ...
                if i == 0:
                    ...
                    plt.plot(t[-2], t[-1], markersize=10 * marker_size_scale, color=target_agent_color, marker='o',
                                markeredgecolor=target_agent_color, zorder=_ZORDER["AGENT"])

            for i in range(len(mapping['dense_agent_labels'])):
                if i == 0:
                    continue
                if frame <= 20:
                    history = mapping['dense_agent_trajs'][:, :frame, :]
                    plt.plot(history[i, :, 0], history[i, :, 1],
                         linestyle="-", color="MediumSlateBlue", marker=None, alpha=1,
                         linewidth=linewidth, zorder=0)
                    plt.plot(history[i, -1, 0], history[i, -1, 1],
                             markersize=10 * marker_size_scale, color="MediumSlateBlue", marker='o',
                             markeredgecolor="MediumSlateBlue")
                    continue
                plt.plot(mapping['dense_agent_trajs'][i, :, 0], mapping['dense_agent_trajs'][i, :, 1],
                         linestyle="-", color="MediumSlateBlue", marker=None, alpha=1,
                         linewidth=linewidth, zorder=0)
                future = mapping['dense_agent_labels'][:, :(frame-19), :]
                temp = np.array([[mapping['dense_agent_trajs'][i, -1, 0], mapping['dense_agent_trajs'][i, -1, 1]],
                                 [mapping['dense_agent_labels'][i, 0, 0], mapping['dense_agent_labels'][i, 0, 1]]])
                plt.plot(temp[:, 0], temp[:, 1],
                         linestyle="-", color="MediumSlateBlue", marker=None, alpha=1,
                         linewidth=linewidth, zorder=0)
                plt.plot(future[i, :, 0], future[i, :, 1],
                         linestyle="-", color="MediumSlateBlue", marker=None, alpha=1,
                         linewidth=linewidth, zorder=0)
                plt.plot(future[i, -1, 0], future[i, -1, 1],
                         markersize=10 * marker_size_scale, color="MediumSlateBlue", marker='o',
                         markeredgecolor="MediumSlateBlue")
        ...
        if frame > 19:
            ...
            for each in predict:
                ...
                if i == 3 and frame == 49:
                    plt.plot(new_each[-1, 0], new_each[-1, 1], markersize=20 * marker_size_scale, color="#007672",
                             marker="*",
                             markeredgecolor='black', zorder=_ZORDER["AGENT"])
                else:
                    plt.plot(new_each[-1, 0], new_each[-1, 1], markersize=10 * marker_size_scale, color="#007672",
                                 marker="o",
                                 markeredgecolor='#007672', zorder=_ZORDER["AGENT"])
                i += 1

            if "visualize_test" not in args.other_params:
                new_labels = np.zeros([len(labels) + 2])
                new_labels[2:] = labels
                label_draw = new_labels[:(frame-19)*2]
                function1 = plt.plot(label_draw[0::2], label_draw[1::2], linestyle="-", color=target_agent_edge_color, linewidth=linewidth, zorder=_ZORDER["AGENT"])
                plt.plot(label_draw[-2], label_draw[-1], markersize=10 * marker_size_scale, color=target_agent_edge_color,
                         marker="o",
                         markeredgecolor=target_agent_edge_color, zorder=_ZORDER["AGENT"])
            if "visualize_test" not in args.other_params:
                functions = function1 + function2
            else:
                functions = function2
    ...
    imgs = []
    listdir = os.listdir(os.path.join(args.log_dir, 'visualize_' + time_begin, get_name("visualize" + ("" if name == "" else "_" + name))))
    ...
    for file in listdir:
        ...
        imgs.append(img)
    last_pos = imgs[18]
    last_pos_list = [last_pos] * 10
    imgs = imgs[:19] + last_pos_list + imgs[19:]
    for i in range(10):
        imgs.append(imgs[-1])
    imageio.mimsave(filename, imgs, format='GIF', fps=10)
