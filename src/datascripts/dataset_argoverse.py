...
def preprocess(args, id2info, mapping):
    ...
    for id in keys:
        ...
        dense_agent_label = []
        for i, line in enumerate(info):
            if i >= 20 and len(info) == 50:
                dense_agent_label.append([line[X], line[Y]])
        for i, line in enumerate(info):
            if larger(line[TIMESTAMP], two_seconds):
                break
            x, y = line[X], line[Y]
            if i > 0:
                vector = [line_pre[X], line_pre[Y], x, y, line[TIMESTAMP], x-line_pre[X], y-line_pre[Y],  line[OBJECT_TYPE] == 'AV',
                          line[OBJECT_TYPE] == 'AGENT', line[OBJECT_TYPE] == 'OTHERS', len(polyline_spans), i]
                vectors.append(get_pad_vector(vector))
            line_pre = line

            if i < 20 and len(info) == 50:
                dense_agent_traj.append([x, y])
            if i < 20 and args.do_test and len(info) == 20:
                dense_agent_traj.append([x, y])
            if i == 1 and len(info) == 50:
                dense_agent_ids.append(len(polyline_spans))
            if i == 1 and args.do_test and len(info) == 20:
                dense_agent_ids.append(len(polyline_spans))
        if len(info) == 50:
            dense_agent_trajs.append(dense_agent_traj)
            dense_agent_labels.append(dense_agent_label)
        if args.do_test and len(info) == 20:
            dense_agent_trajs.append(dense_agent_traj)
    ...
    mapping['dense_agent_trajs'] = np.array(dense_agent_trajs)
    mapping['dense_agent_labels'] = np.array(dense_agent_labels)
    mapping['dense_agent_ids'] = dense_agent_ids
    return mapping
